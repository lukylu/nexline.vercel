...
export function switchPdImg(_thumb: HTMLElement, idx: number) {
  const thumbsEl = document.getElementById('pdThumbs');
  if (!thumbsEl) return;
  const pid = thumbsEl.dataset.pdid || '';
  const p = cachedProducts.find(x => x.id === pid);
  const mainImg = document.getElementById('pdImg') as HTMLImageElement;
  if (!mainImg || !p) return;
  
  const imgs = (p.images && p.images.length > 0) ? p.images : ((productImgs[pid as any] && productImgs[pid as any].length > 0) ? productImgs[pid as any] : [mainImg.src]);
  const src = imgs[idx];
  if (!src) return;
...
export function pdWish() {
  ...
  if (wBtn) wBtn.textContent = wishes.has(state.pdCurrentId as any) ? '♥ En tu wishlist' : '♡ Guardar en wishlist';
}
